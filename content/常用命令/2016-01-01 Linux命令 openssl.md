...
#####################
## 生成证书请求文件
2.1生成csr请求文件首先下载openssl软件，可以去openssl官网下载：http://www.openssl.org/related/binaries.html下载后安装到本地计算机。
2.1.1查看openssl在终端输入openssl version查看openssl当前版本。图1
2.1.2生成key私钥文件使用以下命令来生成私钥：`openssl genrsa -des3 -out www.deedbeef.com.key 2048`，生成的私钥保存在当期目录。
2.1.3生成[根证书]csr文件使用以下命令来生成私钥：`openssl req -new -key www.mydomain.com.key -out www.mydomain.com.csr`
